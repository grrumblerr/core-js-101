...
function isInsideCircle(circle, point) {
  const r1 = point.x - circle.center.x;
  const r2 = point.y - circle.center.y;
  if (r1 ** 2 + r2 ** 2 >= circle.radius ** 2) return false;
  return true;
  // throw new Error('Not implemented');
}
...
function isCreditCardNumber(ccn) {
  const arr1 = ccn.toString().split('').reverse();
  const arr2 = arr1.map((el, i) => {
    if (i % 2 !== 0) {
      if (2 * el > 9) {
        return 2 * el - 9;
      }
  ...
  return arr2.reduce((a, b) => a / 1 + b / 1) % 10 === 0;
  // throw new Error('Not implemented');
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < arr2.length; i += 1) {
    if (err[arr2[i]]) arr.push(arr2[i]);
    else if (pairs[arr.pop()] !== arr2[i]) {
      return false;
    }
  }
...
function evaluateTicTacToePosition(position) {
  for (let i = 0; i < position.length; i += 1) {
    ...
    } else if (position[2][0] === position[1][1] && position[1][1] === position[0][2]) {
      if (position[2][0]) {
        return position[2][0];
      }
    }
